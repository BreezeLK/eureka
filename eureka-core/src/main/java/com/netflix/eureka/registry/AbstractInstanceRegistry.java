...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
    ...
    protected AbstractInstanceRegistry(EurekaServerConfig serverConfig, EurekaClientConfig clientConfig, ServerCodecs serverCodecs) {
        ...
        // 定时调度任务，维护最近变更的服务实例信息，放在recentlyChangedQueue中
    ...
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        ...
        try {
            ...
            //其实这个registry map，就是一个注册表，里面包含了每个服务的每个服务实例的注册信息
            if (gMap == null) {
            ...
            // 刚开始看服务注册的时候，是看不懂这行代码的
            // 抓取注册表的时候，会走多级缓存的机制，当服务实例注册信息变化的时候，就要更新这个缓存
    ...
    protected boolean internalCancel(String appName, String id, boolean isReplication) {
        ...
        try {
            ...
            if (instanceInfo != null) {
                ...
                instanceInfo.setLastUpdatedTimestamp(); // 设置了服务实例的lastUpdatedTimestamp，最近一次变更的时间戳

                vip = instanceInfo.getVIPAddress();
                svip = instanceInfo.getSecureVipAddress();

                // 服务的注册、下线、故障摘除，都会代表这个服务实例化了，都会将自己放入最近改变的队列中去
    ...
    public void evict(long additionalLeaseMs) {
        ...
        for (Entry<String, Map<String, Lease<InstanceInfo>>> groupEntry : registry.entrySet()) {
            ...
            if (leaseMap != null) {
                for (Entry<String, Lease<InstanceInfo>> leaseEntry : leaseMap.entrySet()) {
                    ...
                    // 超过了90 * 2 = 180s的话，才会认为这个服务实例过期了，认为故障了
        ...
        // 不能一次性摘除过多的服务实例
        // 假设现在一共是20个服务实例，现在有6个服务实例不可用了，一次性可以摘除的服务实例
        // 下一次，还剩17个实例 -》 3个不可用 -》 toEvict = 3， 本次可以摘除的就是3个实例
        // 最后剩14个服务实例
        ...
        if (toEvict > 0) {
            ...
            for (int i = 0; i < toEvict; i++) {
                ...
                // 对这个随机挑选出来的服务实例，就调用internalCancel方法摘除
    ...
    /* visible for testing */ class EvictionTask extends TimerTask {
        ...
        long getCompensationTimeMs() {
            // 本来呢，20:02:32才来，不是60s调度执行一次吗？jvm gc,卡顿了，啥都不干了；或者本地linux操作系统的时钟出了问题

            // 4、下一次又过来了，又执行了，20:01:00

            // 1、先获取当前时间
            long currNanos = getCurrentTimeNano();
            // 2、上一次这个EvictionTask被执行的时间，0，将当前时间设置到AtomicLong中去
            // 3、20:00:00，此时就会将20:00:00设置到lastExecutionNanosRef这个里面去

            // 5、发现上次为20:00:00，再次将20:01:00置到lastExecutionNanosRef这个里面去

            // 如果20:02:32才来，会获取上一次的时间:20:00:00
            // 将这一次的时间保存到lastExecutionNanosRef这个里面去,设置进去20:02:32
            ...
            // 6、20:01:00 - 20:00:00 = 60s

            // 20:02:32 - 20:00:00 = 2分钟32秒 = 152秒
            long elapsedMs = TimeUnit.NANOSECONDS.toMillis(currNanos - lastNanos);

            // 7、用这个60s - 配置的60s = 0s

            // 用这个算出来的152秒 - 60秒 = 92秒， =》 比预期的时间晚了92秒
            long compensationTime = elapsedMs - serverConfig.getEvictionIntervalTimerInMs();

            // compensationTime = 92s，返回
